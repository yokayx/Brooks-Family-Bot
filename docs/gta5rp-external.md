...
Каждый проход бот смотрит, не висит ли на нас свежая атака: Brooks в
`defender_name`, `server_name == RICHMAN`, бой не доигран и начался не позже
`VZP_DEF_WINDOW_MINUTES` (45 мин) назад — окно нужно, потому что в списке
`/wars` висят протухшие `active`-записи за несколько дней.

Что делает один раз на бой (помечает `war_id` в `vzp_defenses`):

1. тегает роль VZP и пишет: кто забил, точка, время начала, ссылка на карту
   войны `https://vzp-launcher.pro/vzp?war={id}`;
2. создаёт обычный сбор **без статиков** с заголовком `DEF vs {семья}` и
   кнопками ➕/➖ — тот же механизм, что у `/плюсы`.

Канал — `PLUS_DEF_CHANNEL_ID` в `bot/config.py` (сейчас = каналу итогов ВЗП).
